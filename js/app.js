...
// // 1. Update the RESET function so it explicitly kills the ringing alarm
// function resetTimer() {
//   clearInterval(timerInterval);
//   isTimerRunning = false;
//   timeLeft = 25 * 60;
...
//   // Shut down the ringing alarm loop completely when reset is clicked
//   const alarmAudio = document.getElementById('audioAlarm');
//   if (alarmAudio) {
//     alarmAudio.pause();
//     alarmAudio.currentTime = 0;
//     alarmAudio.loop = false; // Turn off looping state
//   }
// }

// // 2. Update the COMPLETION function to loop the audio continuously without popup freezes
// function handleTimerCompletion() {
//   isTimerRunning = false;
  
//   // Set button state to reflect the alarm is active
//   startPauseBtn.textContent = 'Alarm Active';
//   startPauseBtn.style.backgroundColor = '#ff9f43'; // Warning orange color
...
//     alarmAudio.currentTime = 0;
//     alarmAudio.loop = true; // UPGRADE: Tell the browser to repeat this track endlessly
    
//     alarmAudio.play().catch(() => {
//       console.log("Alarm playback was deferred by the browser environment layer.");
//     });
//   } else {
//     // Legacy Oscillator backup fallback loop if physical file is missing
...
//     oscillator.start();
//     gainNode.gain.exponentialRampToValueAtTime(0.00001, audioCtx.currentTime + 2.0);
//     oscillator.stop(audioCtx.currentTime + 2.0);
//   }
  
//   // REMOVED: The blocking alert() box is completely gone! No more layout freezing.
//   rotateQuote();
// }
...
// --- Pomodoro Timer Engine Variables (Drift-Proof System Clock Tracking) ---
let timerInterval = null;
let timeLeft = 25 * 60; 
let isTimerRunning = false;
let endTime = null; // Precise system millisecond target timestamp
...
function initApp() {
  ...
  rotateQuote(); 
  loadSavedMixerSettings();
  updateTimerDisplay(); // Render initial 25:00 view state cleanly
}
...
function toggleTimer() {
  if (isTimerRunning) {
    ...
    // Set exact target end timestamp based on current real-world clock time plus time left
    endTime = Date.now() + (timeLeft * 1000);
    
    if (timerInterval) clearInterval(timerInterval);
    
    // Run at a fast 200ms polling rate to stay perfectly synced even if the tab sleeps
    timerInterval = setInterval(() => {
      const now = Date.now();
      const remainingDistance = Math.round((endTime - now) / 1000);
...
function handleTimerCompletion() {
  ...
  // Set button state to reflect the alarm is active
  startPauseBtn.textContent = 'Alarm Active';
  startPauseBtn.style.backgroundColor = '#ff9f43'; 
  ...
  if (alarmAudio) {
    alarmAudio.volume = 1.0;
    alarmAudio.currentTime = 0;
    alarmAudio.loop = true; // Loop endlessly until manually stopped via Reset
    
    alarmAudio.play().catch(() => {
      console.log("Alarm playback was deferred by the browser environment layer.");
    });
  } else {
    // Fallback electronic audio context beep sequence if asset is missing
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const oscillator = audioCtx.createOscillator();
    const gainNode = audioCtx.createGain();
    oscillator.connect(gainNode);
